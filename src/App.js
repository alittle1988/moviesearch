...
function App() {
  ...
  function handleAddMovieTitle(movie) {
    //testing to add id to each movie
    let id = moviesList.length + 1;
  ...
  function handleRemoveMovieTitle(title) {
    const data = moviesList.filter(
      (movie) => movie.name.toLowerCase() !== title.name.toLowerCase()
    );
    setMoviesList(data);
...
// working on fetching API in useEffect
