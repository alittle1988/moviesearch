  ...
  function checkTitleMatch(e, title) {
    ...
    moviesList.forEach((movie, index) => {
      if (movie.name.toLowerCase() === title.name.toLowerCase()) {
        if (
          window.confirm("Movie alread exit! Would you like to Edit Movie?")
        ) {
          setTitleExist(true);
          setMovieIndex(index);
          setMovieDetails({
    ...
    //setIsLoading(true)
    //  fetch(`http://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=${title}&format=jsonp`).then(response => console.log(response.json())).catch( alert('There was an Error')).finally(() => {setIsLoading(false)})
  }
      ...
      {showDetail === true ? (
        <div className="addTitle">
          <Form onSubmit={(e) => addTheTitle(e, movieDetails)}>
            <Form.Group>
              <Form.Label
                className="movieInfoTitle"
                style={{ color: "white", marginLeft: 15, fontSize: 30 }}
              >
                Moive Info
              </Form.Label>
              <Form.Control
...
// left off trying to fetch data from wiki onClick checkTitleMatch
